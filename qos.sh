...
if [ $1 = "vcpe-1" ]; then
	...
	sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "192.168.255.5", "nw_proto": "TCP"}, "actions":{"queue": "0"}}' http://localhost:8080/qos/queue/0000000000000001
	echo -e "\nConfigurando la QoS en h12"
	sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "192.168.255.6", "nw_proto": "TCP"}, "actions":{"queue": "1"}}' http://localhost:8080/qos/queue/0000000000000001
	...
	sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "192.168.255.7", "nw_proto": "TCP"}, "actions":{"queue": "0"}}' http://localhost:8080/qos/rules/0000000000000001
	echo -e "\nConfigurando la QoS en h22"
	sudo docker exec -it $VNF1 curl -X POST -d '{"match": {"nw_dst": "192.168.255.8", "nw_proto": "TCP"}, "actions":{"queue": "1"}}' http://localhost:8080/qos/rules/0000000000000001
fi
echo -e "\n"
